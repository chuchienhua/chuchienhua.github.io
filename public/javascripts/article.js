// 將文章輸入至頁面上 供getArticle()呼叫
function initArticle(data) {
    //文章標題及內容
    var edirArticle='';
    var delArticle='';
    if(data.account ==$.cookie('userID')){
        edirArticle=`<a clas="person" href="javascript:showArticlerow()">修改文章</a>`;

        delArticle=`<a class=person href="javascript:delArticle()">刪除文章</a>`;
    }

    var head="<h3>"+data.name+"(<a class='person' \ href='blog.html?account="+data.account +" '>"+data.account+"</a>)"+edirArticle+'  '+delArticle+"</h3>";
    $('#a_head').append(head);
    ...
    var content=`<h2 class='a_title'>${data.title}</h2>
                  <div class='a_head'>
                  <a class='a_type' href='blog.html?type=${data.type}'>${data.type}
                  </a>
                  ${date}
                  </div>
                  <p>${data.content}</p>
    ...
    data.comment.forEach(function (com) {
        ...
        if(com.account == $.cookie('userID')){
            editbtn=
            `<a class="person"
            href="javascript:showEditrow('${com.id}')">
            修改回應
            </a>`;

            delbtn=
            `<a class="person"
            href="javascript:delComment('${com.id}')">
            刪除回應
            </a>`;
        }
        ...
        comment = `<div id="${com.id}">
                       ...
                       <span id="like_count">${com.like.length}</span></a>
	                   ...
	                   <div>
                           ...
                           <div class="comRow" style="border-bottom: 1px solid #dfdfdf;padding: 24px 0;">${com.message}
                           </div>
                           <div class="editRow" style="display:none">
                           <lable for="field6">
...
function getArticleById(){
    ...
    $.get("/blog/getArticleById?_id="+getUrlVal("_id"),
    function(res,status){
        if(res.status !=0){
            location.href='/public/blog.html';
        }else{
...
function editArticle(){
    $.post("/blog/editArticle",
    {
        ...
        'content':$('#a_content .text-field').val()
    },function(res){
        if(res.status ==0){
...
function addcomment(){
    ...
    var message=$('#content').val().replace(/ /g,'&nbsp').replace(/\n/g,"<br />");
